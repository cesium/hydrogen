    ...
    return (
        <button 
            ...
            className = {`relative flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition-colors
                        ${isSelected? ' text-primary ': 'text-gray hover:text-gray-900 hover:bg-gray-100'}` }>
                <span>{icon} </span>
                {name}
                {isSelected && <motion.div
                    layoutId = {layoutId}
                    className = "absolute bottom-0 left-0 w-full bg-primary h-[2px]"
                />}
        </button>
    )
}
...
export function TabsContainer({children, className}: BasicProps){
    return <div className={clsx("flex flex-row p-1 rounded-xl",className)}>{children}</div>
}
...
export function GroupTabs({ defaultTab, layoutId, children, className}: GroupTabsProps){
