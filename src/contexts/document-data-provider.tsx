...
import {DocumentsData} from "@/lib/types"
...
const DocumentsContext  = createContext<DocumentDataContext | undefined>(undefined); 

export function DocumentsDataProvider({children}: {children:React.ReactNode}){
    ...
    return ( <DocumentsContext.Provider 
            value = {{
                yearSelected, 
                setYearSelected,
                documentsData,
...
export function useSetYear() {
    ...
    if (context === undefined){
        throw new Error("selectYear() can only be used insied a DocumentsDataProvider")
    }
...
export function useDocumentsData() {
    const context= useContext(DocumentsContext) 
    if (context === undefined){
        throw new Error("getDocumentsData() can only be used insied a DocumentsDataProvider")
    }
...
export function useFetchingState() {
    const context= useContext(DocumentsContext) 
    if (context === undefined){
        throw new Error("getFetchingState() can only be used insied a DocumentsDataProvider")
    }
